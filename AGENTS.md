...
不要提交企业数据、本地 wiki 内容或真实密钥。`QCC_MCP_TOKEN` 在 `plugin.yaml` 中声明为必需 secret，具体值应放在宿主环境中。默认企业知识库位于仓库外的 `~/.noeticai/company-knowledge`，可用 `NOETICAI_COMPANY_KB_DIR` 覆盖。Kanban 任务工作目录默认为 `~/.noeticai/kanban-runs/<tenant>`，可用 `NOETICAI_KANBAN_RUNS_DIR` 覆盖根目录。
