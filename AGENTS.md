...
本仓库是 Company Work Suite 插件，不是传统应用服务；核心组织单元是可发现的 skill。
...
使用 `--target codex|claude|hermes|work-suite` 做单宿主或单契约校验。修改 `skills/cws-workflow/scripts/workflow_cli.py`、workflow YAML 或插件注册逻辑时，运行集成测试。修改 `gate` 契约或 `check_artifact_gate.py` 时，运行 `test_artifact_gate`；修改跨宿主 delegate runner 时，运行 `test_delegate_runner`。

有 `gate` 的编排节点：Agent 必须先写企业信息库下的 `artifacts/<run-id>/<skill-id>/handoff.json`（根目录同 `CWS_COMPANY_KB_DIR` / `~/.cws/company-knowledge`），并在 handoff 顶层写同一 `run_id`，再跑：
...
Skill 名称使用 `cws-` 前缀和小写 kebab-case，例如 `cws-company-profile`。Workflow stage ID 应短、稳定、语义清晰。
...
不要提交企业数据、本地 wiki 内容或真实密钥。`QCC_MCP_TOKEN` 在 `plugin.yaml` 中声明为必需 secret，具体值应放在宿主环境中。默认企业知识库位于仓库外的 `~/.cws/company-knowledge`，可用 `CWS_COMPANY_KB_DIR` 覆盖；运行时 `artifacts/` 与 `raw/`、`wiki/` 同根落在该目录下。Kanban 任务工作目录默认为 `~/.cws/kanban-runs/<tenant>`，可用 `CWS_KANBAN_RUNS_DIR` 覆盖根目录。
