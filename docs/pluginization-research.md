# Company Work Suite 插件化结论

> 日期：2026-07-03
> 范围：`company-work-suite` 作为多宿主 skills 插件的结构、workflow、agent role skill 分工与企业数据策略。

## 结论摘要

`company-work-suite` 应按 skills-first 插件组织，不做成传统应用服务或独立 workflow runtime。
...
| 层级 | 职责 | 示例 |
| --- | --- | --- |
| Skill | 声明单张知识卡片或最终报告能力 | `cws-company-profile`、`cws-due-diligence` |
| Workflow | 声明前置 stage、artifact 输入输出和并行关系 | `skills/cws-due-diligence/references/workflow.yaml` |
| Agent / Role Skill | 执行 DAG 节点或角色 | 默认 agent、`cws-data-agent`、`cws-gen-agent` |
...
| --- | --- | --- |
| 默认 agent | Hermes Kanban 的通用运行身份，负责接收 task 并加载 Company Work Suite 套件 | 不表达 data/gen 业务角色 |
| `cws-data-agent` role skill | 企业画像、股权结构、司法风险、融资历史等前置知识卡片；负责 wiki-first、QCC fallback、raw/wiki 写回和 `evidence_gaps` | 不生成最终尽调/投分报告，不把缺失字段写成确定结论 |
| `cws-gen-agent` role skill | 企业尽调、投资分析等编排型报告卡片；只综合父任务 artifact | 不重新取数，不绕过父任务数据缺口补事实 |

Hermes planned task 不传 `--assignee`；`data` / `gen` 是 workflow role，通过 `cws-data-agent` / `cws-gen-agent` role skill 表达。

## 跨宿主兼容

`workflow.yaml` 不包含 Hermes Kanban 字段，因此可以被不同宿主解释：

- Hermes：`workflow.yaml -> task DAG -> Kanban task`，planned 节点不指定 assignee，前置节点带 `cws-data-agent`，报告节点带 `cws-gen-agent`。
...
`/cws-workflow` 把编排型 skill 的 `workflow.yaml` 转成任务 DAG。DAG 节点不是新的业务类型，而是一次 skill 执行：
...
| `node.role` / `role_skill` | stage 与 entry skill 推导 | `role` 表达 data/gen；`role_skill` 是 `cws-data-agent` 或 `cws-gen-agent` |
...
```text
task1: data cws-company-profile -> company_profile
task2: data cws-shareholder-structure <- company_profile -> shareholder_structure
task3: data cws-litigation-risk <- company_profile -> litigation_risk
task4: data cws-financing-history <- company_profile -> financing_history
task5: gen  cws-due-diligence <- company_profile + shareholder_structure + litigation_risk + financing_history
```
...
| --- | --- | --- | --- | --- |
| `compile` | `workflow_cli.py compile` | `workflow.yaml` | 打印 DAG JSON，不调用 Hermes | 检查 nodes/edges、调试任务拓扑 |
...
跨宿主预览或无 Hermes 环境时优先 `delegate`，因为它只依赖通用 DAG。CWS agent 任务实际跑在 Hermes 时，优先使用 `planned` 作为标准执行 backend；`auto` 是 Hermes 探索式拆解，不作为业务 SOP 的唯一依据。

### Hermes Kanban 与 LangGraph 对比

当前前提是 CWS agent 任务实际跑在 Hermes。对比重点不是“是否需要 graph”，而是谁来承接任务生命周期、默认 agent 调度、日志、状态和人工接管。
...
| 适用边界 | Hermes 内运行 CWS workflow 的主 backend | Hermes dispatcher 内部实现，或未来 CWS 服务端 runtime |
...
- 如果未来需要动态循环、审批中断、跨天恢复，可以在 Hermes dispatcher 或 CWS 服务端内部试 LangGraph；插件仍只面向 Hermes Kanban contract。
- 不做 `CWS plugin -> LangGraph -> Hermes Kanban` 这种双调度链，除非 Kanban 无法表达真实业务状态。
...
- 默认先查用户本地企业知识库：`~/.cws/company-knowledge`。
- 可用 `CWS_COMPANY_KB_DIR` 覆盖知识库目录。
- 只有 wiki 无命中、字段缺失、主体不确定或信息明显过期时，才调用 QCC 等公开企业信息源。
- 补齐后写入 `raw/`，再整理进 `wiki/`。
- 输出必须包含来源、数据时间、wiki 写回状态和 `evidence_gaps`。

business skill 不需要描述具体数据源控制策略，只需要声明业务输入、`data_needs`、输出 artifact 和质量约束。数据访问、写回和缺口标注由 `cws-data-agent` role skill 和任务模板统一执行。
...
`skills/cws-company-profile/card.yaml` 把企业画像定义为独立知识卡片：
...
`skills/cws-due-diligence/references/workflow.yaml` 声明企业尽调的前置 DAG：
...
- `/cws-workflow` 可把 workflow 编译为 DAG，并支持 Hermes Kanban 或宿主子代理委派。
