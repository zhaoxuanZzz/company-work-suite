...
本文记录 `/cws-workflow execute` 当前试行执行层的 Hermes Kanban 方案。通用 workflow 语义属于 `/cws-workflow`；Hermes Kanban 只负责首版运行 backend。

## 结论

`/cws-workflow` 负责读取编排型 skill 的 `references/workflow.yaml`；Hermes Kanban backend 负责把 workflow 变成任务图执行。

首版不指定 Hermes 执行 profile，Kanban task 由 Hermes 默认 agent 承接；角色分工交给 role skill：

| Profile / Skill | 用途 | 执行什么 | 不做什么 |
| --- | --- | --- | --- |
| 默认 agent | Hermes task 运行身份 | 接收 Kanban task，加载 Company Work Suite 套件 | 不表达业务角色 |
| `cws-data-agent` skill | 前置卡片角色 | 企业画像、股权结构、司法风险、融资历史等中间产物卡片 | 不生成最终报告，不把缺失字段补写成确定结论 |
| `cws-gen-agent` skill | 最终报告角色 | 企业尽调、投资分析等编排型报告卡片 | 不重新取数，不绕过父任务 artifact 补事实 |

不单独拆 `data` 和 `analysis`。现有知识卡片本身已经同时包含数据需求、业务判断和输出格式，强拆会先制造迁移成本。

## 角色职责

`cws-data-agent` 是首版的前置卡片角色 skill。它接收一个具体知识卡片任务，只负责产出该卡片声明的 artifact。

`cws-data-agent` 的输入：

- task body 中的目标公司、输入 artifact 和输出 artifact。
- 父任务 handoff 中的结构化 artifact、来源、数据时间和 `evidence_gaps`。
- 当前 Company Work Suite 套件里的对应 `SKILL.md` 和 `card.yaml`。
...
`cws-gen-agent` 是首版的最终报告角色 skill。它只处理编排型报告卡片，例如 `cws-due-diligence` 和 `cws-investment-analysis`。
...
| --- | --- | --- |
| `cws-due-diligence` | `skills/cws-due-diligence/references/workflow.yaml` | `due_diligence_report` |
| `cws-investment-analysis` | `skills/cws-investment-analysis/references/workflow.yaml` | `investment_analysis_report` |

当前复用的前置卡片：

| Skill | 产物 | Role | Assignee |
| --- | --- | --- | --- |
| `cws-company-profile` | `company_profile` | `data` | 默认 agent |
| `cws-shareholder-structure` | `shareholder_structure` | `data` | 默认 agent |
| `cws-litigation-risk` | `litigation_risk` | `data` | 默认 agent |
| `cws-financing-history` | `financing_history` | `data` | 默认 agent |

这些前置卡片不是纯取数卡片。它们会按 `card.yaml` 的 `data_needs` 检索企业信息库或公开信息，也会产出结构化判断和 `evidence_gaps`。因此首版不按业务拆 profile，而是让 Hermes 默认 agent 执行 task，并用 `cws-data-agent` / `cws-gen-agent` 区分角色。

## Backend 映射

规则尽量静态，避免让模型猜编排：

| CWS workflow | Hermes Kanban backend |
| --- | --- |
...
| `stage.id == report` | `role=gen`，`role_skill=cws-gen-agent` |
| `stage.skills` 包含编排型 skill 本身（终端 stage） | `role=gen`，`role_skill=cws-gen-agent` |
| 其他 stage | `role=data`，`role_skill=cws-data-agent` |
...
来源：`skills/cws-due-diligence/references/workflow.yaml`
...
来源：`skills/cws-investment-analysis/references/workflow.yaml`
...
它和企业尽调共用同一批前置卡片，只是最终 `gen` 任务换成 `cws-investment-analysis`。
...
```text
skills/cws-workflow/scripts/workflow_cli.py
```
...
```bash
python skills/cws-workflow/scripts/workflow_cli.py compile \
  --skill cws-due-diligence \
  --company "杭州XX科技有限公司" \
  --workspace scratch
```
...
`skills/cws-workflow/scripts/workflow_cli.py execute` 不直接运行模型。它先把 workflow 编译成 DAG，再用多条 `hermes kanban create --parent ...` 把节点和边写入 Hermes board。

当前不是把一个 DAG JSON 直接交给 Hermes。Hermes CLI 暂无通用的 `import-dag` / `create-dag` 入口；DAG 能力通过 task parent 关系表达。`hermes kanban swarm` 可以一次创建图，但拓扑固定为 `workers -> verifier -> synthesizer`，不适合 CWS workflow 的可变 stage graph。

运行前不需要准备 CWS 专用 profile。`--apply` 不再检查 `worker` profile，也不会给 `hermes kanban create` 传 `--assignee`；Hermes 按默认 agent 承接任务。业务职责由任务正文中的 `cws-data-agent` / `cws-gen-agent` 约束，不用 profile 名表达角色。
...
# 3. 生成并提交 CWS workflow 任务
python skills/cws-workflow/scripts/workflow_cli.py execute \
  --skill cws-due-diligence \
  --company "小米科技有限责任公司" \
  --workspace dir:/absolute/path/to/cws-run \
  --apply
...
6. 默认 agent 被 dispatcher 启动，按 `cws-gen-agent` role skill 读取父任务 handoff，生成最终报告并 complete。
...
这个 skill 不执行企业分析，只负责把自然语言请求转成 `workflow_cli.py execute --apply`，并按用户选择使用 `planned` 或 `auto` 模式：

```bash
# planned（标准流程）
python /path/to/company-work-suite/skills/cws-workflow/scripts/workflow_cli.py execute \
  --mode planned \
  --skill cws-due-diligence \
  --company "<用户输入的公司名>" \
...
```text
/cws-workflow 帮我对杭州XX科技有限公司跑企业尽调
```

orchestrator skill 做三件事：

1. 识别编排 workflow：企业尽调 -> `cws-due-diligence`，投资分析 -> `cws-investment-analysis`。
2. 识别目标公司和可选 workspace。
3. 调用 `skills/cws-workflow/scripts/workflow_cli.py execute --apply`，把创建出来的 task id 返回给用户。

不建议让普通 `cws-due-diligence` skill 自己创建 Kanban 任务。报告卡片的职责是生成报告；编排 Kanban 是 orchestrator 职责。

Hermes 对话里也可以手工用 `/kanban create ...` 创建单个任务；但完整 CWS workflow 需要多个 task、parent 依赖和固定 task body，用脚本生成更稳。

## 非目标

- 不让 Hermes 原生解析 CWS `workflow.yaml`。
- 不把卡片内部强拆成纯 data / 纯 gen。
- 不做 eval/verifier。
- 不做可视化编排器。
- 不做 CWS 专用 profile 或复杂 profile 白名单。
