...
6. 编排任务必须将可检查产物写入企业信息库目录（与 `raw/`、`wiki/` 同根）：`artifacts/<run-id>/<skill-id>/handoff.json`（可选同目录 `report.md`）。`run_id` 由任务正文提供，且必须写入 handoff 顶层；非编排任务可沿用原有产物路径。根目录为 `CWS_COMPANY_KB_DIR`，未设置时使用 `~/.cws/company-knowledge`。`handoff.json` 必须覆盖业务 skill `card.yaml` 的 `gate` / `outputs` 约定。
